...
![image](https://github.com/user-attachments/assets/9e5ab791-ec9d-4288-8896-631679701f3b)
+ 從第六章開始讀
...
+ 台北資安展web3第三天心得
+ 第一場
	1. frontrun-backrun: sandwich 
	2. ethereum12秒、bsc3秒反應時間：front run
	3. 通常做backrun 
	4. slippage MEV 
	5. Bastet開源專案(https://github.com/OneSavieLabs/Bastet)
+ 第二場(Reentrancy Attack重入攻擊)
	1. 背景：The DAO: 利用rallback 這個function,造成當時財損4.5%ETH
	2. 重入攻擊目前還是前十名攻擊，手段一直在進化
	3. 可重入性不是漏洞，只是程式特性。但漏洞是開發者問題。
	4. 重入：
	5. 重入鎖，不建議自己寫重入鎖，建議是用已寫好的TransientGaurd(坎昆版本以上)或ReentrancyGaurd(更早版本)
	6. 重入鎖還是不夠好，重入鎖還需額外的gas fee，故應該回歸初始概念
	7. 設計開發程式應遵守CEI，但CEI也不夠好，所以近年來有FREI-PI
	8. 因為上述都需要額外的gas,這樣用戶體驗差，故又回到不變性測試(Invariant Test)
	9. 用Bastet開源專案來做不變性檢測 
+ 第三場(跨鏈安全)
  	1. 以太坊的不可能三角選擇安全，十二秒出塊，但犧牲scalability
	2. 所以有了L2, rollup有兩種(Optimistic rollup, zk rollup)
	3. 現有的zk 有些與EVM不相容，對開發體驗會差
<!-- Content_END -->
