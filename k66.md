...
+ EtherGame.sol
  ...
  ```
  ### 2025.04.20
  + 上述`EtherGame`把漏洞的`this.balnace`換成`depositedWei`就可避免漏洞
  + TODO: 第九章後面例子
  + 第十章Tokens
  	+ 名詞
   		+ Attestation: A token can represent a certification
     	+ The ERC20 Token Standard
      		+ 2015.11 第一次被介紹 by Fabian Vogelsteller, as an Ethereum Request for Comments (ERC).
        	+ totalSupply
         	+ balanceOf
          	+ transfer
          	+ transferFrom
