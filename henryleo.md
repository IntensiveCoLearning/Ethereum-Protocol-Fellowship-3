...
椭圆曲线在数学上的形式通常是 $y^3 = x^3 + ax + b$ ，$a, b$ 为常数，且满足$4a^3 + 27b^3 ≠ 0$。

这个表达式是为了确保定义的曲线是“平滑的”，或者更准确地说，**没有奇点**。奇点是指曲线上的特殊点，例如自相交的点或者尖锐的点。当该表达式为零，意味着三次方程有重根，这会影响椭圆曲线的“加法”运算，这个稍后解释。

#### 椭圆曲线的加法

##### 两个不同的点 P 和 Q 的相加 (P + Q = R)

- 在椭圆曲线上标记两个不同的点，分别标记为 P 和 Q
- 画一条直线连接点 P 和点 Q
- 这条直线通常会与椭圆曲线在第三个点相交，把这个交点标记为 R'。
- 最后一步，找到点 R' 关于 x 轴的对称点。这个点 R 就是 P + Q 的结果

![两个不同的点 P 和 Q 的相加 (P + Q = R)](https://aandds.com/blog/images/ecc_add_example1.gif)

##### 一个点 P 与自身相加 (P + P = 2P)

- 找到点 P
- 画一条切线，这条直线恰好在点 P 处与椭圆曲线相切
- 找到另一个交点，这条切线通常会与椭圆曲线在另一个点相交。让我们把这个交点标记为 R'
- 同样地，找到点 R' 关于 x 轴的对称点 R。这个点 R' 就是 P + P 的结果，通常记为 2P

![一个点 P 与自身相加 (P + P = 2P)](https://aandds.com/blog/images/ecc_add_example2.gif)

##### 无穷远点 (O)

还有一个特殊的点叫做无穷远点，通常用 O 表示，在垂直方向无限远的点。

- 任何点 P 加上无穷远点 O 都等于 P：`P + O = P`。
- 如果两个点的 x 坐标相同，但 y 坐标互为相反数（例如 (x, y) 和 (x, -y)），那么它们相加的结果就是无穷远点 O，这是因为连接这两个点的直线是垂直的

### 2025.04.09

#### 模运算 Modulo Operation

`a ≡ b (mod n)` 读作 “a 模 n 同余于 b”。
- `a` 除以 `n` 的余数与 `b` 除以 `n` 的余数相同。
- 或者，`a - b` 可以被 `n` 整除。

例子：`8 ≡ 1 (mod 7)`。因为 8 除以 7 的余数是 1，而 1 除以 7 的余数也是 1。或者，`8 - 1 = 7`，可以被 7 整除。

假设`b`小于`n`，那么`b mod n = b`

#### 模逆元 Modular Inverse

对于整数 `a` 和正整数 `n`，如果存在一个整数 `x`，使得：
`(a * x) ≡ 1 (mod n)`
那么 `x` 就被称为 `a` 模 `n` 的模逆元。

例子：`2 * 4 ≡ 8 ≡ 1 (mod 7)`
- 这里，`a = 2`，`n = 7`，`x = 4`
- 由于 `(2 * 4) mod 7 = 8 mod 7 = 1`，所以 4 是 2 模 7 的模逆元

### 2025.04.10

#### 椭圆曲线的标量乘法

$k * P = P + P + ... + P$ (重复 $k$ 次加法)，这有什么好学的？

当k特别大的时候，计算机无法处理。有一种方法叫做Double-and-Add 双倍与相加：

当计算 $k * P$ 时，先将 $k$ 转化为二进制：$(b_n b_{n-1} ... b_1 b_0)_2$，其中$b_i$为 $0$ 或 $1$ ，则$k*p$为：
$$k * P = b_n * 2^n * P + b_{n-1} * 2^{n-1} * P + ... + b_1 * 2^1 * P + b_0 * 2^0 * P$$
即可通过遍历完成乘法运算：
- 每一步都翻倍
- 当$b_i=1$时，加P，$b_i=0$时，不加P
...
$$R = R + R = O + O = O\tag{1}$$
$$R = R + P = O + P = P \tag{2}$$
(1)式翻倍，$R=O$，没变；(2)式加$P$，根据加法，$R=P$
3. 中间位是 1：
$$R = R + R = P + P = P$$
$$R = R + P = P + P = 2P$$
4. 最低位是 0：
$$R = R + R = 3P + 3P = 6P$$
$b_i$为0，不用加P，所以最终结果为$R=6P$

##### 标量乘法的在加密中重要性

在 ECC 中，用户的公钥是通过将私钥（一个大的随机整数 `d`）与一个固定的基点 `G` 进行标量乘法得到的：`Public Key = d * G`

### 2025.04.11
#### 椭圆曲线离散对数问题

Elliptic Curve Discrete Logarithm Problem 椭圆曲线离散对数问题，简称 ECDLP，其表述为：已知椭圆曲线上的两个点 `G` 和 `P`，找到一个整数 `d`，使得 `P = d * G` 成立。换句话说就是怎么找到私钥 `d`

这个问题的复杂度目前依然是指数级的，随着密钥长度的增加，解决问题所需的时间会急剧增加，所以ECDLP 是 ECC 安全性的基础。
...
Group 群 是一个集合，连同一个定义在这个集合上的二元运算（通常称为“乘法”或“加法”），满足一些特定的性质（或称为公理），公式化为：$(G, \space *)$ ，其中 $G$ 为集合， $*$ 为二元运算。群是了解椭圆曲线配对的预备知识。

其中，二元运算 $*$ 要满足四个条件：
- Closure 闭包性：假设 $a, b$ 是 $G$ 内的元素，$a*b$ 必须也为 $G$ 内的元素
- Associativity 结合律：假设 $a, b, c$ 是 $G$ 内的元素，满足 $(a * b) * c = a * (b * c)$
- Identity Element 单位元：群中存在一个特殊的元素，常记作$e, \space 0$ ，满足：
    - 当运算符为 $*$ ：$a * e = e * a = a$
    - 当运算符为 $+$ ：$a + 0 = 0 + a = a$
    - 总的来说就是加了白加、乘了白乘
- Inverse Element 逆元：类似于抵消的意思
    - $a * a^{-1} = a^{-1} * a = e$
    - $a + (-a) = (-a) + a = 0$
...
- 逆元：对于任何整数 $a$，它的逆元是 $-a$，因为 $a + (-a) = (-a) + a = 0$

椭圆曲线密码学： 椭圆曲线上的点，连同无穷远点 $O$ 和定义的加法规则，也构成一个群：
- 闭包性：两个点相加的结果仍然是曲线上的一个点（或无穷远点）
- 结合律：虽然证明比较复杂，但椭圆曲线上的加法满足结合律
- 单位元：无穷远点 $O$ 是单位元，因为 $P + O = O + P = P$
- 逆元：对于曲线上的一个点 $P = (x, y)$，它的逆元是 $-P = (x, -y)$，因为 $P+ (-P) = O$

#### 阿贝尔群

在群的基础上同时满足Commutativity 交换律则被称为 Abelian Group 阿贝尔群
...
Elliptic Curve Pairings 椭圆曲线配对 是椭圆曲线密码学中一个更高级的概念。它是一种特殊的函数，可以将椭圆曲线上的两个点映射到另一个数学群中的一个元素，并且这个映射具有一些非常有用的特性。
...
Bilinear Map 双线性映射是椭圆曲线配对的核心。

先看看什么是线性！线性指的是双线性映射/函数 $e$ 对其每个输入参数分别保持群的结构。假设点 $P, Q$ 属于椭圆曲线群 $G_{1}$ ，点$R$ 属于群 $G_{2}$ ，且满足：
$$e(P + Q, R) = e(P, R) * e(Q, R)$$

其中 $e(P + Q, R)$ 是群 $G_{T}$ 上的点。

- $e(P + Q, R)$：第二个参数 $R$ 不变，变化的是第一个参数 $(P + Q)$，在群 $G_{1}$ 中将 $Q$ 和 $R$ 进行**加法群操作**；然后通过函数 $e$ 将两个参数映射至 $G_{T}$
- $e(P, R) * e(Q, R)$：表示分别通过函数 $e$ 将 $P$ 和 $Q$ 与 $R$ 映射至 $G_{T}$，再在  $G_{T}$ 中进行**乘法群操作**
- 假设这个 $e$ 使得式子成立，即**函数 $e$ 使得原本在 $G_{1}$中的加法群操作转换为在$G_{T}$中的乘法群操作，这就是线性的意思！**


双线性映射就是两个参数都有这种线性的性质。

##### 双线性性质示例
$$e(aP, bQ) = e(P, Q)^{ab}$$
首先看第一个参数，是椭圆曲线的标量乘法，相当于 $P$ 连加 $a$ 次：
$$e(aP, Q) = e(P + ... + P, Q)$$
再根据线性：
$$原式= e(P, Q) * e(P, Q) * ... * e(P, Q)=e(P, Q)^{a}$$
同理，第二个参数：
$$e(P, bQ) = e(P, Q)^{b}$$
综上：
$$e(aP, bQ) = e(aP, Q)^{b} = (e(P, Q)^{a})^{b} = e(P, Q)^{ab}$$

其实就是一个函数，它接受两个“东西”作为输入，并以一种可预测的方式将它们组合起来，输入的时候可能是乘法，输出的时候又变成了开方。
...
Boneh-Lynn-Shacham BLS 签名，也称BLS短签名，顾名思义其主要特点是生成的签名非常短，这在带宽受限的环境中非常有用。

BLS基于椭圆曲线配对，但有一些密码学常用的东西是通用的：
- 私钥 $sk$：一个大整数
- 公钥：$pk = sk * G$，$G$ 是椭圆曲线的元点
- message 消息：需要被验证的字符串，记作`m`，被哈希后记为$H(m)$
- 签名：$sig = sk * H(m)$，椭圆曲线的标量乘法

#### 签名验证
$$e(sig, G) == e(H(m), pk) \tag{1}$$
只要(1)式为真，则确定消息为真。

根据椭圆曲线配对的性质：$e(aP, Q)=e(P, aQ)$
$$e(sig, G) = e(sk * H(m), G) = e(H(m), sk * G)$$
又因为 $pk = sk * G$ ：
...
以128 位安全强度为例：
- RSA 需要 3072 Bit 位，签名大小也就是 $3072 \div 8 = 384 \space Byte$ 
- ECDSA 通常使用 256 位的私钥和在 256 位椭圆曲线上的点，描述一个点需要两个坐标，坐标大小与曲线的位数相当，故：$2 \times 256 = 512 \space bit = 64 \space Byte$
- BLS 也需要一个椭圆曲线上的点，但长度大小和安全强度大致相当，即 $128 \times 2 = 256 \space bit = 32 \space Byte$

#### 基于BLS的签名聚合

Signature Aggregation 签名聚合，是BLS的一个应用，将多个不同消息的签名聚合为一个单一的短签名

假设有签名 $sig_{1}$ 和 $sig_{2}$ ，有：
$$sig_{1} = sk_{1} * H(m_{1})$$
$$sig_{2} = sk_{2} * H(m_{2})$$
聚合即相加：
$$AggSig = sig_{1} + sig_{2} = sk_{1} * H(m_{1}) + sk_{2} * H(m_{2})$$
验证方法也随之变化：
$$e(AggSig, G) == e(H(m_{1}), pk_{1}) * e(H(m_{2}), pk_{2}) \tag{2}$$


### 2025.04.16

今天花时间研究zk-SNARKs，之前从概念上理解了，但好像没办法和双线性配对结合，再研究看看
...
工作流：
![2025041701](https://miro.medium.com/v2/resize:fit:1188/format:webp/1*YD-ckgBfmmmRBCyVlhp8YQ.png)

#### 将声明转化为算术电路

Algebraic Circuit 计算电路，一个算术电路可以被看作是一个由一系列的gates 门和连接这些门的wires 线 组成的计算网络，线是值，门是操作。以下以一个例子说明：

- 声明：我知道一个数字 $x$，使得 $x * x + 5 = y$ (其中 $y$ 是公开的)。
- 设 $y = 14$
- 当且仅当存在一个 $x$ 满足这个等式时，电路的输出为真

第一步，识别输入和输出：
- 输入线：
    - $x$ ：这是证明者知道的秘密输入（也称为witness 见证）
    - $y$ ：这是公开的输入，这里设为14
- 输出线：等式 $x * x + 5 = y$ 成立，一般为了方便会将项丢到一边：$x * x + 5 - y = 0$
...
2. 计算 $x * x + 5$
    - 输入： $w_{1}$ 和 常量 $5$ 
    - 加法门
    - 输出：$w_{2} = w_{1} + 5 = x * x + 5$
3. 检查 $x * x + 5 - y = 0$
    - 输入：常量 $7$ 和 $w_{2}$ 和 $y = 14$ 
    - 减法门
    - 输出：$output = w2 - y = x * x + 5 - 14 = x * x - 9$

第三步，电路的输出：算术电路的输出是 $output$ 线上的值，假设它$= 0$ ，则为真
...
R1CS 秩为 1 的约束系统，用一组三元组 $(a_{i}, b_{i}, c_{i})$ 来对应一个约束，该约束必须对一个变量向量 $s$ 成立

$$(a_{i} \cdot  s) * (b_{i} \cdot s) = (c_{i} \cdot s) \tag{*}$$

##### 示例：算术电路改写为R1CS

以刚刚的例子来看，向量 $s$ 可以为：
$$s = [1, x, y, w_{1}, w_{2}, output]$$
而约束为：
$$w_{1} = x^{2} = x * x \tag{1}$$
$$w_{2} = w_{1} + 5 \tag{2}$$
$$output = w_{2} - y \tag{3}$$
...
要使其与 式 $(a_{i} \cdot  s) * (b_{i} \cdot s) = (c_{i} \cdot s) $ 一起成立，即：
$$
\left \{
\begin{array}{l}
a_{1} \cdot s = x \\
...
$$a_{1} = [0, 1, 0, 0, 0, 0]$$
使得：
$$a_{1}\cdot s = 0 \cdot 1 + 1 \cdot x + 0 \cdot y + 0 \cdot w_{1} + 0 \cdot w_{2} + 0 \cdot output = x$$
同理，为了得到第二个 $x$ ，有：
$$b_{1} = [0, 1, 0, 0, 0, 0] = a_{1}$$
$$b_{1}\cdot s = x$$

为了得到 $w_{1}$ ，$c_{1} = [0, 0, 0, 1, 0, 0]$ 。
...
对于 $(2)$ 式，可改写为 $(w_{1} + 5) * 1 = w2$ ，有：
$$
\left \{
\begin{array}{l}
a_{2}  = [5, 0, 0, 1, 0, 0] = w_{1} + 5 \\
b_{2}  = [1, 0, 0, 0, 0, 0] = 1 \\
c_{2}  = [0, 0, 0, 0, 1, 0] = w_{2} \\
\end{array}
\right.
$$

对于 $(3)$ 式，可改写为 $w_{2} = output + y$ ，细看是 $w_{2} \cdot 1 = output + 14$ （$y = 14$），有：
$$
\left \{
\begin{array}{l}
a_{3}  = [0, 0, 0, 0, 1, 0] = w_{2} \\
b_{3}  = [1, 0, 0, 0, 0, 0] = 1 \\
c_{3}  = [14, 0, 0, 0, 0, 1] = output + 14 \\
\end{array}
\right.
$$
...
对于声明 $x * x + 5 = 14$，有变量向量 $s = [1, x, y, w_{1}, w_{2}, output]$ ，和以下约束：
- 设
$$
\left \{
\begin{array}{l}
...
- 则有
$$
\begin{bmatrix} A \\ B \end{bmatrix} \cdot s 
= 
\begin{bmatrix} x \cdot x \\ (w_{1} + 5) \cdot 1  \\ w_{2} \cdot 1 \\ \end{bmatrix}
= 
C \cdot s 
=
\begin{bmatrix} w_{1} \\ w_{2} \\ output + 14 \end{bmatrix}
$$

为了证明原始声明成立，我们需要找到一个 $x$ 的值，使得当所有中间变量 ($w_{1}, w_{2}, output$) 也被正确计算出来时，所有的 R1CS 约束都得到满足，并且最终的输出 $output$ 等于 0
