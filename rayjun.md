...
Ethereum 本质还是一个分布式系统，每个节点通过 p2p 网络与其他节点相连。以太坊中的 p2p 网络协议的实现就是 devp2p。

devp2p 有两个核心功能，一个是节点发现，让节点在接入网络时能够与其他节点建立联系；另一个是数据传输服务，在与其他节点建立联系之后，就可以想换交换数据。

*ethdb + triedb*

ethdb 完成以太坊数据存储的抽象，提供统一的存储接口，底层的数据库可以是 leveldb，也可以 pebble。

有些数据可以通过 ethdb 接口直接对底层数据库进行读写，但是以太坊中还有很大一块的数据是状态数据，这些数据会被组织成 MPT 的结构，也就是 geth 中 的 trie 结构，在以太坊协议运行的过程中，trie 数据会有很多的中间状态，这些数据不能直接调用 ethdb 进行读写，需要 triedb 来管理这些数据和中间状态，最后才通过 ethdb 来持久化。

*EVM*

EVM 是以太坊这个状态机的状态转换函数，所有状态数据的更新都只能通过 EVM 来进行，网络可以接受到交易和区块信息，这些信息被 EVM 处理之后会成为状态数据库的一部分。

### 2025.04.19
整理了一下以太坊 Geth 源码的架构图，Geth 的代码量虽然很多，但是其中很多代码都是辅助代码或者单元测试，如果只关注协议的核心实现，那么就需要重点关注 core、eth、ethdb、node、p2p、rlp、trie & triedb 等模块。

如果理解了以太坊本身的职能，那么对于 Geth 源码的划分就比较清晰了，关注三个核心部分：网络、计算和存储，计算部分相对独立，就是 EVM 的实现，网络和存储的部分实现复杂，网络在 devp2p 的基础之上实现了以太坊自身的各种协议和功能，而存储的核心则是对于 MPT 的实现，其实就是对于状态数据的读写操作：
