...
**EIP-6780**：同一交易中仅执行 SELFDESTRUCT

`SELFDESTRUCT` 操作码的行为将发生变化，以适应未来的 Verkle trees 和帐户版本化。在新的规则下，`SELFDESTRUCT` 将只在同一交易中创建和销毁合约的情况下保留其原有的行为；在其他情况下，它将只转账资金而不删除合约账户。任何依赖 `SELFDESTRUCT` 来升级合约或燃烧以太的行为都将不再有效12，17。此外，`SELFDESTRUCT` 被明确标记为弃用，不鼓励在新合约中使用8。

## 2025.04.18

**EIP-7044**：永久有效签名自愿退出，在卡佩拉上锁定自愿退出签名域以实现永久有效性

EIP-7044 提案主张在 Capella 分叉中锁定验证者自愿退出签名域，以实现永久有效性。EIP-7044 的动机在于当前签名的自愿退出仅对两次升级有效，这通过信标链状态考虑当前和上一个分叉版本来限制。该提案的目的是为了简化某些质押操作，特别是在质押者和资金所有者不同时。此外，EIP-7044 还调整了 `voluntary_exit` 的 gossip 条件，并确保了向后兼容性。

## 2025.04.19

**EIP-7045**：增加最大验证包含槽位，将最大验证包含槽位增加到包含验证槽位的 N+1 周期的最后一个槽位，其中 N 是包含验证槽位的周期。

EIP-7045 旨在将区块链验证器投票的最大包含时隙从`attestation.slot + SLOTS_PER_EPOCH`增加到包当前 epoch 和下一 epoch 之间的最后一个时隙。这一改变对于保证链高的安全性和确认规则至关重要，同时也对执行层和共识层的标准追踪引入了改变。该提议尤为关键的一环是对 LMD-GHOST 安全协议的修正，以及对区块确认时间的缩短。此外，该提案还调整了验证器的参与奖，允许在更长的时间范围内接收奖励，从而提高了链高的安全性。

## 2025.04.20

**EIP-7514**：添加最大 Epoch 流失限制

**EIP-7514** 旨在通过引入最大 chopped 频率来限制每个时代周期内可被激活的验证器数量，以缓 slowed 下加入太多验证器的增长率。

**EIP-7516**：BLOBBASEFEE 指令

**EIP-7516** 引入了 `BLOBBASEFEE` 指令，用于获取当前数据 blob 的基础费用，是 `BASEFEE` 指令的补充，专门针 object blob 数据结构，有助于智能合约处理数据 blob 相关的费用。
<!-- Content_END -->
