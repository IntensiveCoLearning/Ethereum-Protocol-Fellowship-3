...
### 2025.03.26
#### MEV
什麼是 MEV: MEV 指的是區塊提議者（Proposer）、礦工（PoW 時代）、驗證者（PoS 時代）或其他市場參與者，在交易排序時可以額外獲取的利潤

MEV 的運作方式: 在區塊鏈上，交易並非按照用戶提交的順序直接執行，而是由區塊提議者決定交易的排列方式。這種排序權力帶來了 MEV 機會，主要來源包括：
- 三明治攻擊（Sandwich Attack）：在 AMM（如 Uniswap）上，攻擊者發現用戶的大額交易，然後：
  - 先插入一筆「買入」交易，推高價格
  - 讓用戶的交易以較高價格成交
  - 再插入一筆「賣出」交易，獲利
- 清算套利（Liquidation Arbitrage）：在 DeFi 借貸協議（如 Aave, MakerDAO）中，攻擊者監測即將被清算的貸款，並搶先執行清算交易以獲取手續費或折扣資產
- 搶先交易（Front-running）：當有用戶提交高價購買 NFT 或代幣時，機器人可以監測交易池，搶先提交相同交易，使原用戶的交易失敗或被迫以更高價格購買

什麼是區塊鏈的抗審查性（Censorship Resistance）: 抗審查性是區塊鏈最核心的特性之一，代表任何人都可以自由地提交交易，且這些交易應該被公平處理。然而，隨著 MEV 的出現和區塊提議者的集中化，區塊鏈面臨越來越嚴重的審查風險

審查風險的來源
- 提議者/建構者（Proposer/Builder）集中化
- 監管壓力
- 區塊提議者的財務動機

### 2025.03.29
#### Purge 和 Portal Network
什麼是 History Expiry（歷史數據過期）: 歷史數據過期的概念是指 以太坊節點不再永久保存所有歷史數據，而是定期刪除舊數據，以減少儲存需求並提升節點運行效率

為什麼要讓歷史數據過期:
- 區塊鏈數據量過大: 以太坊的區塊鏈數據量持續增長，目前完整節點（Full Node）需存儲超過 數百 GB 的數據，這對運行節點的用戶來說是極大的負擔
- 提高節點的可運行性: 讓更多人可以運行節點，降低去中心化網路的參與門檻
- 優化數據存取: 大多數用戶只關心最近的交易狀態，而非數年前的歷史交易，因此舊數據可移除至次級存儲方案

歷史數據過期如何運作:
- 在一定區塊高度之前的數據將不再被節點存儲，例如只保留過去 1 年 的交易數據
- 歷史數據仍然可以透過外部網路（如 Portal Network）取得，而不是由以太坊節點直接提供

什麼是 Portal Network: Portal Network 是一種 點對點（P2P）覆蓋網路（Overlay Network），專門用來幫助 輕量級客戶端（Light Clients） 存取 當前與歷史區塊鏈數據

Portal Network 的目標:
- 輕量級客戶端不需要存儲完整區塊鏈數據，但仍能查詢區塊、交易、合約狀態等資訊
- 降低以太坊節點的存儲負擔，讓全節點可以安全地「忘記」舊數據，而用戶仍能透過 Portal Network 查詢歷史資料

Portal Network 如何運作: Portal Network 由 多個子網絡（Sub-networks） 組成，每個子網絡負責存儲與提供不同類型的數據
- History Network（歷史數據網路）：提供過去的區塊與交易資訊
- State Network（狀態數據網路）：允許查詢特定帳戶或智能合約的狀態
- Block Header Network（區塊頭網路）：幫助輕量級客戶端快速驗證區塊鏈狀態
- Transaction Network（交易網路）：提供最新的交易資訊
  
Purge（歷史數據清除）+ Portal Network 的影響: 
- 優點
  - 減少節點存儲需求：讓完整節點不需要存儲過多歷史數據，使得運行節點更容易
  - 提高以太坊的可擴展性：避免節點因存儲壓力過大而退出網路，保持去中心化
  - 讓輕量級客戶端更容易存取數據：透過 Portal Network，用戶可以查詢歷史交易與狀態，而無需運行完整節點
- 挑戰
